...
import { uint8ArrayToString } from "@paperbits/common/utils";
...
export class ApiService {
    ...
    private async fetchSpecs(): Promise<any[]> {
        const blobKeys = await this.azureBlobStorage.listBlobs();

        const specPromises = [];

        for (const blobKey of blobKeys) {
            const specPromise = this.azureBlobStorage.downloadBlob(blobKey);
            specPromises.push(specPromise);
        }

        const blobs = await Promise.all(specPromises);
        return blobs.map(x => JSON.parse(uint8ArrayToString(x)));
    }
    ...
    public async getApis(searchQuery?: SearchQuery): Promise<Page<Api>> {
        ...
        page.value = apis.map(x => new Api(x));
    ...
    public async getOperation(apiName: string, operationName: string): Promise<Operation> {
        ...
        const operation = new Operation(operations.find(x => x.name === operationName));
